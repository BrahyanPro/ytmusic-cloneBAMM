    ...
    const res = await fetch('https://accounts.spotify.com/api/token', {
      ...
      headers: {
        ...
        Authorization:
          'Basic ' +
          new Buffer(
            `${'d609d3fe447241839098c8aa532d9157'}:${'2a68b16d45b3403a81532daf9e0bdb2b'}`
          ).toString('base64'),
      },
...
export const getArtistData = async function (artist) {
  try {
    ...
    const backgroundImg = await axios(
    ...
    const biography = await axios(
      `https://es.wikipedia.org/api/rest_v1/page/summary/${artist}`
    );

    const background =
      backgroundImg.data.artists !== null &&
      backgroundImg.data.artists[0].strArtistFanart !== null
        ? backgroundImg.data.artists[0].strArtistFanart
        : info.data.artists.items[0].images[0].url;

    const artistData = {
      artist: info.data.artists.items[0].name,
      followers: new Intl.NumberFormat('es-AR').format(
        info.data.artists.items[0].followers.total
      ),
...
export const getAlbumData = async function (artist, albumTitle) {
  try {
    const album = await axios(
      `https://api.spotify.com/v1/search?q=${artist} ${albumTitle}&type=track&limit=1`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (album.data.tracks.items.length === 0)
      throw new Error('Album no disponible...');
    ...
    const tracks = albumTracks.data.items.map((track) => {
      return {
        type: 'album',
...
const getRandomTopArtist = async function () {
  try {
    const artists = await axios(
      `https://api.spotify.com/v1/search?q=genre:"rock"&type=artist&limit=50`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
...
const getArtistId = async function (artist) {
  try {
    ...
    if (artistId.data.artists.items.length === 0)
...
const getAlbums = async function (artist) {
  try {
    ...
    const albums = await axios(
      `https://api.spotify.com/v1/artists/${artistId}/albums?market=US&limit=30`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
    ...
    return albums.data.items
      ...
      .filter((element) =>
        hash[element.album] ? false : (hash[element.album] = true)
      ); // Eliminar albumes duplicados.
  } catch (error) {
      ...
      return {
        ...
        followers:
          new Intl.NumberFormat('es-AR').format(
            +item.data.artists.items[0].followers.total
          ) + ' suscriptores',
      };
    });
  } catch (error) {
...
const getTopAlbums = async function () {
  try {
    ...
    const artistPromise = artists.map((artist) =>
      axios(
        `https://api.spotify.com/v1/search?q=${artist}&type=artist&limit=1`,
        {
          method: 'GET',
    ...
    const albumsPromise = artistsData.map((artist) =>
      axios(
        `https://api.spotify.com/v1/artists/${artist.data.artists.items[0].id}/albums?market=US&limit=3`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
